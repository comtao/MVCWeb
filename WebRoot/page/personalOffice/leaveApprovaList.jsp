		...
		<div class="container-fluid">
			<div class="row-fluid">
					...
					<table class="table">
						...
						<c:forEach var="list" items="${map.list}">
							<tr>
								...
								<c:choose>
								  <c:when test="${list.status==0}">
								    <a href="#search" onclick="searchAjax('${ctx}','${list.id}','${list.dp_id}','${list.user_id }');" data-toggle="modal" title="审批"><i class="icon-legal"></i>
								    </a>
								  </c:when>
								  <c:otherwise>
								    <a href="${ctx}/Leave/leaveDetail.html?id=${list.id}&userId=${list.user_id}">查看</a>
								  </c:otherwise>
								</c:choose> 
								</td>
							</tr>
				...
				<div class="pagination">
					<ul>
				        <li ><a href="${ctx}/Leave/leaveApprovaList.html?pageNum=1&realName=${roleName}">首  页</a></li>
				        <li><a href="${ctx}/Leave/leaveApprovaList.html?pageNum=${map.page-1 }&realName=${roleName}">上一页</a></li>
				        <li><a href="${ctx}/Leave/leaveApprovaList.html?pageNum=${map.page+1 }&realName=${roleName}">下一页</a></li>
				        <li><a href="${ctx}/Leave/leaveApprovaList.html?pageNum=${map.count }&realName=${roleName}">尾 页</a></li>
			...
			<div class="modal leave hide fade" id="search" tabindex="-1"
					...
					<div class="modal-header">
						...
						<h3 id="myModalLabel">审批请假单</h3>
					</div>
					<div class="modal-body">
						<form action="${ctx}/Leave/leaveApprovaEdit.html"  method="post">
							...
							<table class="table table-bordered">
								<tbody>
									...
									<tr>
										<th>备注</th>
										<td> 
											<textarea  id="bAjax"    name="remarks" rows="5" cols="50" style="width: 579px; height: 30px;"></textarea>
										</td>
									</tr>
									<tr>
										<th>审批</th>
										<td> 
											<input  clas="btn" name=status type="radio" value="1" checked="checked">通过<input  name=status value="2" type="radio" >不通过
										</td>
									</tr>
								</tbody>
							</table>
							<input  type="submit" class=btn value="审批">
						</form>
					</div>
				</div>	
